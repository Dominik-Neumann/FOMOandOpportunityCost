...
topattractive <- tail(descriptive_message_attractive, 10)
topattractive$ranking <- "top"
bottomattractive <- head(descriptive_message_attractive, 10)
bottomattractive$ranking <- "bottom"
dat_attractiveactivities <- rbind(topattractive, bottomattractive)
```

<b>Figure 2.3</b> Means of perceived attractiveness in descending order for both conditions
```{r, fig.width = 8, fig.height = 5}
ggplot(dat_attractiveactivities, aes(x =reorder(message, mean), y = mean, fill = ranking)) +
...
toppopular <- tail(descriptive_message_popular, 10)
toppopular$ranking <- "top"
bottompopular <- head(descriptive_message_popular, 10)
bottompopular$ranking <- "bottom"
dat_popularactivities <- rbind(toppopular, bottompopular)
```

<b>Figure 2.3</b> Means of perceived attractiveness in descending order for both conditions
```{r, fig.width = 8, fig.height = 5}
ggplot(dat_popularactivities, aes(x = reorder(message, mean), y = mean, fill = ranking)) +
  geom_bar(position = "dodge", stat = "identity") +
  apatheme +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  xlab("Message") +
...
topunique <- tail(descriptive_message_unique, 10)
topunique$ranking <- "top"
bottomunique <- head(descriptive_message_unique, 10)
bottomunique$ranking <- "bottom"
dat_uniqueactivities <- rbind(topunique, bottomunique)
```

<b>Figure 2.3</b> Means of perceived attractiveness in descending order for both conditions
```{r, fig.width = 8, fig.height = 5}
ggplot(dat_uniqueactivities, aes(x = reorder(message, mean), y = mean, fill = ranking)) +
  geom_bar(position = "dodge", stat = "identity") +
  apatheme +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  xlab("Message") +
...
topfamiliar <- tail(descriptive_message_familiar, 10)
topfamiliar$ranking <- "top"
bottomfamiliar <- head(descriptive_message_familiar, 10)
bottomfamiliar$ranking <- "bottom"
dat_familiaractivities <- rbind(topfamiliar, bottomfamiliar)
```

<b>Figure 2.3</b> Means of perceived attractiveness in descending order for both conditions
```{r, fig.width = 8, fig.height = 5}
ggplot(dat_familiaractivities, aes(x = reorder(message, mean), y = mean, fill = ranking)) +
  geom_bar(position = "dodge", stat = "identity") +
  apatheme +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  xlab("Message") +
...
topselfrelevance <- tail(descriptive_message_selfrelevance, 10)
topselfrelevance$ranking <- "top"
bottomselfrelevance <- head(descriptive_message_selfrelevance, 10)
bottomselfrelevance$ranking <- "bottom"
dat_selfrelevanceactivities <- rbind(topselfrelevance, bottomselfrelevance)
```

<b>Figure 2.3</b> Means of perceived attractiveness in descending order for both conditions
```{r, fig.width = 8, fig.height = 5}
ggplot(dat_selfrelevanceactivities, aes(x = reorder(message, mean), y = mean, fill = ranking)) +
  geom_bar(position = "dodge", stat = "identity") +
  apatheme +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  xlab("Message") +
...
topexperiencedfomo <- tail(descriptive_message_experiencedfomo, 10)
topexperiencedfomo$ranking <- "top"
bottomexperiencedfomo <- head(descriptive_message_experiencedfomo, 10)
bottomexperiencedfomo$ranking <- "bottom"
dat_experiencedfomoactivities <- rbind(topexperiencedfomo, bottomexperiencedfomo)
```

<b>Figure 2.3</b> Means of perceived attractiveness in descending order for both conditions
```{r, fig.width = 8, fig.height = 5}
ggplot(dat_experiencedfomoactivities, aes(x = reorder(message, mean), y = mean, fill = ranking)) +
  geom_bar(position = "dodge", stat = "identity") +
  apatheme +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  xlab("Message") +
...
toprepeatability <- tail(descriptive_message_repeatability, 10)
toprepeatability$ranking <- "top"
bottomrepeatability <- head(descriptive_message_repeatability, 10)
bottomrepeatability$ranking <- "bottom"
dat_repeatabilityactivities <- rbind(toprepeatability, bottomrepeatability)
```

<b>Figure 2.3</b> Means of perceived attractiveness in descending order for both conditions
```{r, fig.width = 8, fig.height = 5}
ggplot(dat_repeatabilityactivities, aes(x = reorder(message, mean), y = mean, fill = ranking)) +
  geom_bar(position = "dodge", stat = "identity") +
  apatheme +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  xlab("Message") +
...
```{r}
model_fomo <- lmer(scale(experiencedfomo) ~ condition * fomo.constr + familiar + selfrelevance + repeatability + ( 1 | id ), data = dat)
summary(model_fomo)
```

```{r}
model_attractive <- lmer(scale(attractive) ~ condition * fomo.constr + familiar + selfrelevance + repeatability + ( 1 | id ), data = dat)
summary(model_attractive)
```

```{r}
model_unique <- lmer(scale(unique) ~ condition * fomo.constr + familiar + selfrelevance + repeatability + ( 1 | id ), data = dat)
summary(model_unique)
```

```{r}
model_popular <- lmer(scale(popular) ~ condition * fomo.constr + familiar + selfrelevance + repeatability + ( 1 | id ), data = dat)
summary(model_popular)
```

```{r results = FALSE}
table <- sjPlot::tab_model(model_fomo, model_attractive, model_unique, model_popular)
```

## Multilevel replication

```{r}
model_fomo2 <- lme(scale(experiencedfomo) ~ fomo.constr * selfrelevance * condition, random = ~1 | id, data = dat, na.action = na.exclude)

fomoupper <- mean(dat$fomo.constr, na.rm = TRUE) + sd(dat$fomo.constr, na.rm = TRUE)
fomolower <- mean(dat$fomo.constr, na.rm = TRUE) - sd(dat$fomo.constr, na.rm = TRUE)
selfrelevanceupper <- mean(dat$selfrelevance, na.rm = TRUE) + sd(dat$selfrelevance, na.rm = TRUE)
selfrelevancelower <- mean(dat$selfrelevance, na.rm = TRUE) - sd(dat$selfrelevance, na.rm = TRUE)

effectsfomoxrelevance <- effect(term = "fomo.constr:selfrelevance:condition", xlevels= list(fomo.constr = c(fomolower, fomoupper), selfrelevance = c(selfrelevancelower, selfrelevanceupper)), mod = model_fomo2)
dat_effectsfomoxrelevance <- as.data.frame(effectsfomoxrelevance)

dat_effectsfomoxrelevance$fomo.constr <- as.factor(dat_effectsfomoxrelevance$fomo.constr)
dat_effectsfomoxrelevance$selfrelevance <- as.factor(dat_effectsfomoxrelevance$selfrelevance)
dat_effectsfomoxrelevance$fomo.constr <- recode_factor(dat_effectsfomoxrelevance$fomo.constr,'2.03252497411602' = "-1SD Trait-FOMO", '3.82648665379096' = "+1SD Trait-FOMO")
dat_effectsfomoxrelevance$selfrelevance <- recode_factor(dat_effectsfomoxrelevance$selfrelevance, '2.47096776015285' = "-1SD Self-relevance", '8.6515086866843' = "+1SD Self-relevance")

ggplot(dat_effectsfomoxrelevance, aes(x = selfrelevance, y = fit, color = fomo.constr, group = fomo.constr)) + 
    geom_point() + 
    geom_line(size = 1.2) +
    labs(title = "Threeway interaction", x = "Self-relevance", y = "Experienced FOMO", color = "Trait-FOMO", fill = "Trait-FOMO") +
  theme_classic() +
  facet_wrap(~condition)
```

```{r}
model_fomo3 <- lme(scale(experiencedfomo) ~ fomo.constr * attractive * condition, random = ~1 |id, data = dat, na.action = na.exclude)

fomoupper <- mean(dat$fomo.constr, na.rm = TRUE) + sd(dat$fomo.constr, na.rm = TRUE)
fomolower <- mean(dat$fomo.constr, na.rm = TRUE) - sd(dat$fomo.constr, na.rm = TRUE)
attractiveupper <- mean(dat$attractive, na.rm = TRUE) + sd(dat$attractive, na.rm = TRUE)
attractivelower <- mean(dat$attractive, na.rm = TRUE) - sd(dat$attractive, na.rm = TRUE)

effectsfomoxattractive <- effect(term = "fomo.constr:attractive:condition", xlevels= list(fomo.constr = c(fomolower, fomoupper), attractive = c(attractivelower, attractiveupper)), mod = model_fomo3)
dat_effectsfomoxattractive <- as.data.frame(effectsfomoxattractive)

dat_effectsfomoxattractive$fomo.constr <- as.factor(dat_effectsfomoxattractive$fomo.constr)
dat_effectsfomoxattractive$attractive <- as.factor(dat_effectsfomoxattractive$attractive)
dat_effectsfomoxattractive$fomo.constr <- recode_factor(dat_effectsfomoxattractive$fomo.constr,'2.03252497411602' = "-1SD Trait-FOMO", '3.82648665379096' = "+1SD Trait-FOMO")
dat_effectsfomoxattractive$attractive <- recode_factor(dat_effectsfomoxattractive$attractive, '2.75209164221774' = "-1SD Attractiveness", '8.78547544186097' = "+1SD Attractiveness")

ggplot(dat_effectsfomoxattractive, aes(x = attractive, y = fit, color = fomo.constr, group = fomo.constr)) + 
    geom_point() + 
    geom_line(size = 1.2) +
    labs(title = "Threeway interaction", x = "Attractiveness", y = "Experienced FOMO", color = "Trait-FOMO", fill = "Trait-FOMO") +
  theme_classic() +
  facet_wrap(~condition)
```

```{r}
model_fomo4 <- lme(scale(experiencedfomo) ~ fomo.constr * popular * condition, random = ~1 |id, data = dat, na.action = na.exclude)

fomoupper <- mean(dat$fomo.constr, na.rm = TRUE) + sd(dat$fomo.constr, na.rm = TRUE)
fomolower <- mean(dat$fomo.constr, na.rm = TRUE) - sd(dat$fomo.constr, na.rm = TRUE)
popularupper <- mean(dat$popular, na.rm = TRUE) + sd(dat$popular, na.rm = TRUE)
popularlower <- mean(dat$popular, na.rm = TRUE) - sd(dat$popular, na.rm = TRUE)

effectsfomoxpopular <- effect(term = "fomo.constr:popular:condition", xlevels= list(fomo.constr = c(fomolower, fomoupper), popular = c(popularlower, popularupper)), mod = model_fomo4)
dat_effectsfomoxpopular <- as.data.frame(effectsfomoxpopular)

dat_effectsfomoxpopular$fomo.constr <- as.factor(dat_effectsfomoxpopular$fomo.constr)
dat_effectsfomoxpopular$popular <- as.factor(dat_effectsfomoxpopular$popular)
dat_effectsfomoxpopular$fomo.constr <- recode_factor(dat_effectsfomoxpopular$fomo.constr,'2.03252497411602' = "-1SD Trait-FOMO", '3.82648665379096' = "+1SD Trait-FOMO")
dat_effectsfomoxpopular$popular <- recode_factor(dat_effectsfomoxpopular$popular, '2.84647272632792' = "-1SD Popularity", '8.71077235417297' = "+1SD Popularity")

ggplot(dat_effectsfomoxpopular, aes(x = popular, y = fit, color = fomo.constr, group = fomo.constr)) + 
    geom_point() + 
    geom_line(size = 1.2) +
    labs(title = "Threeway interaction", x = "Popularity", y = "Experienced FOMO", color = "Trait-FOMO", fill = "Trait-FOMO") +
  theme_classic() +
  facet_wrap(~condition)
```